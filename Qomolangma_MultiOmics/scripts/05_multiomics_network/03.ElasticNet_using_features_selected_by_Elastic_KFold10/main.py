...
def train_elasticnet_model(X: pd.DataFrame, y: pd.Series, cv_folds: int = 10, 
                         random_state: int = 42) -> Tuple[ElasticNet, Dict, Dict, StandardScaler]:
    """训练ElasticNet模型并进行交叉验证"""
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        elastic_net = ElasticNet(max_iter=10000, tol=1e-4, random_state=random_state)
        param_grid = {'alpha': alphas, 'l1_ratio': l1_ratios}
        
        grid_search = GridSearchCV(
            elastic_net, param_grid, cv=cv_folds, scoring='r2', n_jobs=-1, error_score='raise'
        )
        ...
        coefficients.append(dict(zip(X_train_processed.columns, best_model.coef_)))
    ...
    return grid_search.best_estimator_, avg_metrics, coefficients, scaler

def predict_and_evaluate(model: ElasticNet, X: pd.DataFrame, y: pd.Series, 
...
def calculate_average_coefficients(coefficients: List[Dict], features: List[str]) -> Dict:
    """计算特征的平均系数"""
    avg_coefficients = {}
    for feature in features:
        coefs = [coef_dict.get(feature, 0) for coef_dict in coefficients]
        non_zero_coefs = [c for c in coefs if abs(c) > 1e-8]
        avg_coefficients[feature] = np.mean(non_zero_coefs) if non_zero_coefs else 0
...
def process_metabolite_prediction(X_train: pd.DataFrame, y_train: pd.Series, 
    ...
    if cached_results is None:
        ...
        model, cv_metrics, coefficients, scaler = train_elasticnet_model(X_train_subset, y_train)
        ...
        avg_coefficients = calculate_average_coefficients(coefficients, stable_features)
        
        model_path = os.path.join(metabolite_data_dir, 'elasticnet_model.pkl')
        ...
        avg_coefficients = cached_results['avg_coefficients']
...
def main(args):
    ...
    setup_logging(os.path.join(log_dir, 'elasticnet_prediction.log'))
    
    # 创建输出目录
    create_directories([output_dir, figure_dir])
    
    logging.info("="*80)
    logging.info("ElasticNet预测分析开始")
    logging.info("="*80)
    ...
    try:
        ...
        for metabolite in tqdm(existing_y_features, desc="Processing metabolites"):
            ...
            if result is not None:
                ...
                for feature, coef in result['avg_coefficients'].items():
        ...
        coefficients_df.to_csv(os.path.join(output_dir, f'{input_filename}_feature_coefficients.csv'), index=False)
        logging.info(f"特征系数已保存至: {os.path.join(output_dir, f'{input_filename}_feature_coefficients.csv')}")
