...
def manual_predict_from_posterior(model_metadata, old_subjects_data):
    ...
    try:
        ...
        if 'Intercept' not in available_params:
            print(f"警告: {site}-{microbe} 缺少Intercept")
            return None
        ...
        if spline_key not in available_params:
            print(f"警告: {site}-{microbe} 缺少样条参数")
            return None
        ...
        if 'sigma' not in available_params:
            print(f"提示: {site}-{microbe} 未找到sigma，使用默认值1")
        ...
        print(f"失败: {site}-{microbe}: {error_msg}")
        return None
...
print(f"\n基本统计:")
print(f"  预测的微生物数: {counterfactual_df['microbe'].nunique()}")
...
site_summary.columns = ['n_microbes', 'n_obs', 'mean_plasticity', 'sd_plasticity', 'n_divergent', 'prop_divergent']
print(site_summary)
...
# 微生物水平汇总
microbe_summary = counterfactual_df.groupby(['site', 'microbe']).agg({
    'time_numeric': 'count',
...
microbe_summary['proportion_divergent'] = (
    microbe_summary['n_divergent_adj'] / microbe_summary['n_timepoints']
).round(3)

microbe_summary = microbe_summary.sort_values('mean_plasticity', key=abs, ascending=False)
microbe_summary.to_csv(OUTPUT_DIR / "counterfactual_summary_by_microbe.csv")
...
print("\n老年人显著高于年轻人预期 (Top 5):")
top_positive = microbe_summary.nlargest(5, 'mean_plasticity')[
    ['mean_plasticity', 'n_divergent_adj', 'proportion_divergent']
]
print(top_positive)

print("\n老年人显著低于年轻人预期 (Top 5):")
top_negative = microbe_summary.nsmallest(5, 'mean_plasticity')[
    ['mean_plasticity', 'n_divergent_adj', 'proportion_divergent']
]
print(top_negative)
...
print(f"  微生物汇总: counterfactual_summary_by_microbe.csv")
