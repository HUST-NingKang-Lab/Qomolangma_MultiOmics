...
files <- c(
  oral = "data/processed/oral_species.csv",
  skin = "data/processed/skin_species.csv",
  gut  = "data/processed/gut_species.csv"
)
...
datasets <- lapply(names(files), function(dataset_name) {
  ...
  data <- read.csv(path, check.names = FALSE, row.names = 1)
