...
# 读取差异分析结果
de_dir = "data/processed"
gut_DE = pd.read_csv(de_dir / "gut_DE_results.csv").query("qval < 0.05")
oral_DE = pd.read_csv(de_dir / "oral_DE_results.csv").query("qval < 0.05")
skin_DE = pd.read_csv(de_dir / "skin_DE_results.csv").query("qval < 0.05")

# 读取丰度表
abs_dir = "data/processed"
...
gut_abs = load_abundance(abs_dir / "gut_species.csv", gut_DE, people, time12)
oral_abs = load_abundance(abs_dir / "oral_species.csv", oral_DE, people, time12)
skin_abs = load_abundance(abs_dir / "skin_species.csv", skin_DE, people, time12)

print(f"加载的微生物数: Gut={len(gut_abs.columns)-2}, Oral={len(oral_abs.columns)-2}, Skin={len(skin_abs.columns)-2}")
...
def preprocess_data(abs_data, site_name, time_mapping, pair_subject_age):
    ...
    # 添加伪计数并log转换
    df = abs_data.copy()
    df[microbe_cols] = np.log(df[microbe_cols] + 1e-8)
    ...
    df_long = (
        df_long
        .groupby(['subject_id', 'time', 'microbe'], as_index=False)
        ['log_abundance']
    ...
    df_long['microbe'] = df_long['microbe'].astype('category')
...
# ==================== 处理三个部位 ====================
print("\n处理各部位数据...")
gut_processed = preprocess_data(gut_abs, 'gut', time_mapping, pair_subject_age)
oral_processed = preprocess_data(oral_abs, 'oral', time_mapping, pair_subject_age)
skin_processed = preprocess_data(skin_abs, 'skin', time_mapping, pair_subject_age)

# 合并所有数据
all_data = pd.concat([gut_processed, oral_processed, skin_processed], 
                     ignore_index=True)

# ==================== 数据质量检查 ====================
print("\n数据质量检查:")
print(f"总观测数: {len(all_data):,}")
print(f"显著微生物数: {all_data['microbe'].nunique()}")
...
# ==================== 保存预处理数据 ====================
output_path = OUTPUT_DIR / "preprocessed_data_for_counterfactual.csv"
...
summary_stats.to_csv(OUTPUT_DIR / "preprocessing_summary.csv")
print("摘要统计已保存")
