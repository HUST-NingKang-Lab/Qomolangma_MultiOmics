...
print(f"  微生物数: {young_only_data['Metabolite'].nunique()}")

combinations = young_only_data[['site', 'Metabolite']].drop_duplicates().reset_index(drop=True)
print(f"\n需要拟合 {len(combinations)} 个年轻人专属模型")

# ==================== 带重试的拟合函数 ====================
def fit_young_only_model_with_retry(idx, site, Metabolite, young_data, output_dir, max_retries=2):
    """带重试机制的模型拟合"""
    
    for attempt in range(max_retries + 1):
        try:
            result = fit_young_only_model_core(idx, site, Metabolite, young_data, output_dir, attempt)
            if result['success']:
                return result
            # 如果是数据问题（非技术错误），不重试
...
def fit_young_only_model_core(idx, site, Metabolite, young_data, output_dir, attempt=0):
    """核心拟合逻辑"""
    
    # 筛选数据
    model_data = young_data[
        (young_data['site'] == site) & 
        (young_data['Metabolite'] == Metabolite)
    ].copy()
    ...
    # 保存
    save_path = Path(output_dir) / f"idata_{idx:06d}_{site}_{Metabolite}.nc"
...
with open("young_only_models_metadata.pkl", 'wb') as f:
...
summary_df.to_csv( "young_models_summary.csv", index=False)
...
if failed_models:
    ...
    failed_df.to_csv( "young_models_failed.csv", index=False)
